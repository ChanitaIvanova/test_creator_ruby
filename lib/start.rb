...
class GraficInterface
  ...
  def self.test(count)
      ...
      GraficInterface.ask(index,test_questions,count)
  end

  def self.open_ask(index,open_question,test_questions,count)
    $f2.destroy if($f2)
    $f2 = TkFrame.new($f1) do
      background "blue"
      pack('expand' => 1,'fill' => 'both','side'=>'bottom')
  ...
  def self.closed_ask(index,closed_question,test_questions,count)
    $f2.destroy if($f2)
    $f2 = TkFrame.new($f1) do
      background "blue"
      pack('expand' => 1,'fill' => 'both','side'=>'bottom')
  ...
  def self.ask(index, test_questions,count)
    $f2.destroy if($f2)
    $f2 = TkFrame.new($f1) do
      background "blue"
      pack('expand' => 1,'fill' => 'both','side'=>'bottom')
    ...
    if (question.class == ClosedQuestion) then
      GraficInterface.closed_ask(index,question,test_questions,count)
    else
      GraficInterface.open_ask(index,question,test_questions,count)
    end
  end
end